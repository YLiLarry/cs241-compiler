...
(provide fst-pass snd-pass)

; (struct Word (val) #:transparent)
        ...
        (lambda ([line : String]) 
            (let ([leftover (filter remove-labels (scan line))])
                (cond [(empty? leftover) #f]
                      [else (inst->mips (tokens->inst leftover tb))]
                )
            )
        ) 
        (string-split str "\n")
...
(define (label-table-from-input in)
    ...
    (define (get-pairs tokens curr)
        (: make-pairs (Any Natural -> (Pair Symbol Natural)))
        (define (make-pairs chars n) 
            (cond 
                [(list? chars)
                    (cons 
                        (string->symbol 
        ...
        (: make-pairs-a (Any -> (Pair Symbol Natural)))
        (define (make-pairs-a chars) (make-pairs chars curr))
        (: make-pairs-b (Any -> (Pair Symbol Natural)))
        (define (make-pairs-b chars) (make-pairs chars (+ 4 curr)))
        (match tokens
            [(list (token 'label x) ..1 _ ..1) (map make-pairs-a x)]
            [(list (token 'label x) ..1) (map make-pairs-b x)]
...
(print-label-table (fst-pass (read-input)))
